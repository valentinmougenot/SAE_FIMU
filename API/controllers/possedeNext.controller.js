...
const Possede = db.possedeNext;
const Op = db.Sequelize.Op;

export const create = (req, res) => {
    if (!req.session.identifiant) {
        res.status(401).send({
            message: "Vous devez être connecté pour créer un lien entre un artiste et un réseau social"
        });
        return;
    }
...
export const update = (req, res) => {
    if (!req.session.identifiant) {
        res.status(401).send({
            message: "Vous devez être connecté pour modifier un lien entre un artiste et un réseau social"
        });
        return;
    }
    const id = req.params.id;
 ...
 export const deleteByIdArtiste = (req, res) => {
    if (!req.session.identifiant) {
        res.status(401).send({
            message: "Vous devez être connecté pour supprimer un lien entre un artiste et un réseau social"
        });
        return;
    }
    if (req.session.role !== "Administrateur") {
        res.status(401).send({
            message: "Vous devez être administrateur pour supprimer un lien entre un artiste et un réseau social"
        });
        return;
    }
 ...
 export const deleteAll =(req, res) => {
    if (!req.session.identifiant) {
        res.status(401).send({
            message: "Vous devez être connecté pour supprimer tous les liens entre un artiste et un réseau social"
        });
        return;
    }
    if (req.session.role !== "Administrateur") {
        res.status(401).send({
            message: "Vous devez être administrateur pour supprimer tous les liens entre un artiste et un réseau social"
        });
        return;
    }
